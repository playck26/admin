...
/**
 * SPEC-062/D3, D2a-1, D6 — **"Avisos do clube", nas configurações.**
 *
 * **Cópia deliberada do componente do `cliente`**, com o visual desta casa.
 * Poly-repo sem pacote compartilhado (ADR-001): o custo é declarado, e é o
 * mesmo do `comprimir-imagem.ts`, que a planta técnica já registra. Não há
 * gate de sincronia — se um dos dois mudar, o outro não fica sabendo.
 *
...
const TEXTO: Record<EstadoDoPush, { titulo: string; detalhe: string }> = {
  ...
  desligado: {
    ...
    detalhe: "Ligue para saber de reserva nova sem abrir o painel.",
  },
  erro: {
      ...
      <div className="rounded-xl border border-[var(--color-outline)] bg-[var(--color-surface)] p-6">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <h2 className="text-lg font-bold">Avisos do clube</h2>
            <p className="mt-1 text-sm text-[var(--color-on-surface-variant)]">
              No iPhone, os avisos só funcionam com o PlayCK instalado na tela
              de início. Toque em Compartilhar e depois em &quot;Adicionar à
              Tela de Início&quot;.
            </p>
          </div>
        </div>
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
            ...
            <button
              ...
              className={`relative mt-1 h-7 w-12 shrink-0 rounded-full transition-colors disabled:opacity-50 ${
                ligadoAgora
                  ...
                  : "bg-[var(--color-on-surface-variant)]/30"
              }`}
            >
              <span
                aria-hidden
          ...
          {estado === "erro" && (
            <div
              role="status"
              className="mt-3 flex flex-wrap items-center gap-3 rounded-lg border border-[var(--color-outline)] px-4 py-3 text-sm font-medium text-[var(--color-on-surface-variant)]"
            >
              <span>
              ...
              <button
                ...
                className="rounded-lg bg-[var(--color-primary-strong)] px-3 py-1 text-sm font-bold text-white"
              >
                Tentar de novo
              </button>
            </div>
          ...
          {ligadoAgora && (
            <button
              ...
              disabled={ocupado}
              onClick={() => void testar()}
              className="mt-4 rounded-lg bg-[var(--color-primary-strong)] px-4 py-2 text-sm font-bold text-white disabled:opacity-60"
            >
