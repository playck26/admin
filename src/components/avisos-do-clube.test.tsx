 ...
 *
 * Espelho do teste do `cliente`, com o mesmo conjunto de casos. A cópia é
 * deliberada (ADR-001, poly-repo sem pacote compartilhado) e **não há gate de
 * sincronia**: se um dos dois ganhar um caso, o outro não fica sabendo.
 *
...
describe("D6 — o aviso de teste", () => {
  ...
  it.each([
    ...
    reconciliar.mockResolvedValue({ estado: "ligado" });
    // O `ApiError` do Admin tem `conflictWith` no 3o parametro; o `code` e o
    // 4o. Copiar a chamada do `cliente` sem olhar teria compilado errado —
    // foi o `tsc` que pegou, nao a leitura.
    pedirAvisoDeTeste.mockRejectedValue(
      new ApiError(409, "qualquer coisa", undefined, code),
    );
    render(<AvisosDoClube />);
    const botao = await screen.findByRole("button", { name: /aviso de teste/i });
