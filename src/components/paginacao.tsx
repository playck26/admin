 ...
 * ---
 *
 * **Copiado do `cliente` na SPEC-065**, porque a caixa de avisos é o mesmo
 * componente nos dois fronts e precisava da mesma dependência. Poly-repo sem
 * pacote compartilhado (ADR-001), custo declarado — como o
 * `netlify-ignore.mjs` e o `gates-de-push.mjs`.
 *
 * **As listas que já existiam no `admin` NÃO foram migradas para cá.** Cada
 * uma tem a sua paginação inline (`students-list.tsx`,
 * `cadastros-pendentes.tsx`), e trocá-las seria refatoração sem pedido, no
 * meio de outra entrega. Fica como dívida nomeada: **existem dois jeitos de
 * paginar no `admin` agora**, e o novo é este.
 */
