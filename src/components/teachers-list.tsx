    ...
    <div className="flex flex-col gap-6 py-6">
      <div className="flex items-center justify-between">
        ...
        <Button asChild className="h-10 gap-2 px-5 text-[13px] font-semibold">
          <Link href="/pessoas/professores/novo">
      ...
      {loading ? (
        ...
        <p className="text-[var(--color-on-surface-variant)]">Nenhum professor cadastrado ainda.</p>
      ) : (
        <div className="overflow-hidden rounded-2xl border border-border bg-[var(--color-surface-container-lowest)] shadow-[var(--shadow-low)]">
          <div className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {data.map((professor) => (
                  <TableRow key={professor.id} className="border-border">
                    <TableCell className="font-medium text-[var(--color-on-surface)]">{professor.nome}</TableCell>
                    <TableCell className="text-[var(--color-on-surface-variant)]">
                      {professor.email ?? professor.telefone ?? "—"}
                    </TableCell>
                    <TableCell>
                      <StatusBadge ativo={professor.status === "ativo"} />
                    </TableCell>
